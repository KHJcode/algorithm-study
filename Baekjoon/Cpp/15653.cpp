...
{
    return (a.first == b.first && a.second == b.second && c.first == d.first && c.second == d.second) || vst[b.first][b.second][d.first][d.second];
}
...
int main()
{
    ...
    while (!que.empty())
    {
        ...
        b = cr.second.first;
        r = cr.second.second;
        vst[r.first][r.second][b.first][b.second] = true;
        if (gph[r.first][r.second] == 2)
        {
            cout << cr.first;
            res = true;
            break;
        ...
        if (!nb.first && !check(r, nr.second, b, nb.second))
        {
